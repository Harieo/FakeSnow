...
public class Events implements Listener {
	...
	public void onWorldLoad(WorldLoadEvent e) {
		RegionManager manager = WorldGuard.getInstance().getPlatform().getRegionContainer()
				.get(BukkitAdapter.adapt(e.getWorld()));
		if (manager != null) {
	...
	public void addRegionToCache(ProtectedRegion region, World world) {
		if (region instanceof GlobalProtectedRegion) {
			FakeSnow.get().getRegionWorldCache().put(world, region);
			return;
		}
		...
		for (int i = min.getBlockX(); i < max.getBlockX(); i += 16) {
			for (int k = min.getBlockZ(); k < max.getBlockZ(); k += 16) {
				...
				FakeSnow.get().getRegionChunkCache().computeIfAbsent(coords, ignored -> Sets.newHashSet()).add(region);
			}
		}
	}
